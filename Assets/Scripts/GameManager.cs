...
public class GameManager : MonoBehaviour {
    ...
    private enum GameState {Initializing, Playing, Losing, Winning, GameOver, GameWin };
	...
	void Update () {
        switch (state)
        {
            ...
            case GameState.Losing:
                ...
                if (backgroundMusic.volume <= 0.0f)
                {
                    AudioSource.PlayClipAtPoint(loseNoticeMusic,
		...
		case GameState.Winning:
                ...
                if (backgroundMusic.volume <= 0.0f)
                {
                    AudioSource.PlayClipAtPoint(winNoticeMusic,
                        gameObject.transform.position);
            ...
            case GameState.GameOver:
                ...
                loseNoticeText.enabled = true;
                winNoticeText.enabled = false;
                break;

            case GameState.GameWin:
                gameOverCanvas.SetActive(true);
                winNoticeText.enabled = true;
                loseNoticeText.enabled = false;
                break;
        }
	}
    ...
    private class TurnCommand : Command
    {
        ...
        public TurnCommand(GameManager gm, int[] turnedCardsIndexes)
        {
            this.gm = gm;
    ...
    private class EliminateCommand : Command
    {
        ...
        public EliminateCommand(GameManager gm, int firstCardIndex, int secondCardIndex)
        {
            this.gm = gm;
